...
public class Legacy
{
    ...
    static void DoBackGuns(TR2Level level)
    {
        ...
        {
            var model = level.Models[_laraSkinGuns4];
            {
                var shifted = _reader2.Read("tr3shifted.tr2");
                var shotty = shifted.Models[(TR2Type)(int)TR3Type.LaraShotgunAnimation_H];
                shotty.Meshes = [shotty.Meshes[14]];
    ...
    public const int _laraGym1 = 0;
    public const int _laraClassic1 = 1;
    public const int _laraMauled = 2;
    public const int _laraCombo1 = 3;
    public const int _laraGold1 = 4;
    public const int _laraBacon = 5;
    public const int _laraGold2 = 6;
    public const int _laraGym2 = 7;
    public const int _laraClassic2 = 8;
    public const int _laraDiving = 9;
    public const int _laraBomber = 10;
    public const int _laraRobe = 11;
    public const int _laraVegas = 12;
    public const int _laraGold3 = 13;
    public const int _laraGym3 = 14;
    public const int _laraClassic3 = 15;
    public const int _laraCoastal = 16;
    public const int _laraLondon = 17;
    public const int _laraNevada = 18;
    public const int _laraAntarc = 19;
    public const int _laraLeigh = 20;
    public const int _laraLeighGold = 21;
    public const int _laraDivingAlpha = 22;
    public const int _laraNGage = 23;
    public const int _laraAntarcBeta = 24;
    public const int _laraBomberAlpha = 25;
    public const int _laraYoung = 26;
    public const int _laraYoungGold = 27;
    public const int _laraClassic4 = 28;
    public const int _laraGold4 = 29;
    public const int _laraNatla = 30;
    public const TR2Type _laraSkin1 = (TR2Type)270;
    public const TR2Type _laraSkin2 = (TR2Type)271;
    public const TR2Type _laraSkinExtra = (TR2Type)272;
    public const TR2Type _laraSkinGuns1 = (TR2Type)273;
    public const TR2Type _laraSkinGuns2 = (TR2Type)274;
    public const TR2Type _laraSkinGuns3 = (TR2Type)275;
    public const TR2Type _laraSkinGuns4 = (TR2Type)339;
    public const TR2Type _laraSkinGuns = (TR2Type)276;
    public const TR2Type _laraSkinLegs = (TR2Type)277;
    public const TR2Type _laraJoints1 = (TR2Type)340;
    public const TR2Type _laraJoints2 = (TR2Type)341;
    public const TR2Type _laraJoints3 = (TR2Type)342;
    public const TR2Type _laraJoints4 = (TR2Type)343;
    ...
    static void SplitModels(TR2Level level)
    {
        ...
        {
            int max = oldType == _laraSkin1 ? 16 : 15;
    ...
    public static TR2Level Run(bool pad)
    {
        ...
        if (true)
        {
...

            {
                ...
                Import(baseLevel, baseModel2, fathoms, fathoms.Models[TR2Type.Lara], tr2Head);
                map.Add([.. baseModel2.Meshes.GetRange(baseModel2.Meshes.Count - 15, 15)]);
            }

            {
                ...
                Import(baseLevel, baseModel2, ngage, ngage.Models[TR2Type.LaraSnowmobAnim_H], tr1Head);
                map.Add([.. baseModel2.Meshes.GetRange(baseModel2.Meshes.Count - 15, 15)]);
            }

            {
                ...
                Import(baseLevel, baseModel2, fathoms, fathoms.Models[TR3Type.Lara], tr3Head);
                map.Add([.. baseModel2.Meshes.GetRange(baseModel2.Meshes.Count - 15, 15)]);
            }

            {
                ...
                Import(baseLevel, baseModel2, skidoo, skidoo.Models[TR2Type.Lara], tr2Head);
                map.Add([.. baseModel2.Meshes.GetRange(baseModel2.Meshes.Count - 15, 15)]);
            }
            ...
            {
                // 30. Natla
                var natla = _reader2.Read(@"Resources\natla.tr2");
                Import(baseLevel, baseModel2, natla, natla.Models[(TR2Type)332], null);
                map.Add([.. baseModel2.Meshes.GetRange(baseModel2.Meshes.Count - 15, 15)]);
            }
            ...
            if (true)
            {
                ...
                {
                    // Give Natla a backpack
                    var nat = map[_laraNatla][7];
                    var lar = map[_laraClassic1][7];
                    var mm = new Dictionary<ushort, ushort>
                    {
                        [10] = 6,
                        [11] = 7,
                        [6] = 5,
                    ...
                    var vs = new ushort[] { 10, 28, 11, 29, 6, 26, 7, 27 };
                    var vmap = new Dictionary<ushort, ushort>();
                    foreach (var v in vs)
                    {
                        if (mm.TryGetValue(v, out ushort value))
                        {
                            vmap[v] = value;
                            continue;
                        }
                        var vert = lar.Vertices[v].Clone();
                        if (v == 28 || v == 29)
                            vert.Y += 6;
                        vmap[v] = (ushort)nat.Vertices.Count;
                        nat.Vertices.Add(vert);
                        nat.Normals.Add(lar.Normals[v]);
                    }

                    var fs = lar.TexturedFaces.Where(f => f.Vertices.All(vs.Contains));
                    foreach (var f in fs)
                    {
                        var nf = f.Clone();
                        nf.Vertices = nf.Vertices.Select(v => vmap[v]).ToList();
                        if (nf.Type == TRFaceType.Triangle)
                            nat.TexturedTriangles.Add(nf);
                        else
                            nat.TexturedRectangles.Add(nf);
                    }

                    nat.TexturedRectangles.RemoveAll(f => f.Vertices.All(v => v >= 4 && v <= 8));

                    // Sort out her neck
                    for (int i = 22; i < 26; i++)
                        nat.Vertices[i].Z += 4;

                    // Annoying skin texture on backpack edge
                    var ffs = new[] { 203, 204, 205, 206 };
                    var atlas = (ushort)baseLevel.Images16.Count;
                    {
                        var seg = new TRImage("Natla/203.png");
                        var tile = new TRImage();
                        tile.Import(seg, new());
                        baseLevel.Images16.Add(new() { Pixels = tile.ToRGB555() });
                        baseLevel.Images8.Add(new() { Pixels = tile.ToRGB(baseLevel.Palette) });
                    ...
                    foreach (var face in nat.TexturedFaces.Where(f => ffs.Contains(f.Texture)))
                    {
                        var tex = baseLevel.ObjectTextures[face.Texture].Clone();
                        tex.Atlas = atlas;
                        tex.Position = new();
                        baseLevel.ObjectTextures.Add(tex);
                        face.Texture = (ushort)(baseLevel.ObjectTextures.Count - 1);
                    }

                    // Double-sided hair bits
                    var p = new ushort[] { 29,30,39,40,41};
                    nat = map[_laraNatla][14];
                    var fac = nat.TexturedTriangles.FindAll(f => f.Vertices.All(p.Contains));
                    foreach (var f in fac)
                    {
                        var n = f.Clone();
                        n.SwapVertices(0, 1);
                        nat.TexturedTriangles.Add(n);
                    }
                }
            }
            ...
            {
                // Natla
                var natla = _reader2.Read(@"Resources\natla.tr2").Models[(TR2Type)332];
                for (int i = 0; i < natla.MeshTrees.Count; i++)
                {
                    baseModel2.MeshTrees[211 + i] = natla.MeshTrees[i].Clone();
                }
            }
